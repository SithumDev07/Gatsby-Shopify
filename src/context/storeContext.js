...
export const StoreProvider = ({ children }) => {
    ...
    const addVariantToCart = async (product, quantity) => {
        ...
        try {
            ...
            let updatedCart = [];
            if(cart.length > 0){
                const itemsInCart = cart.find((item) => item.product.variants[0]?.shopifyId === variantID);
    ...
    const removeLineItem = async (variantId) => {
        ...
        try {
            ...
            setLoading(false)
            console.error("Error in removeLineItems");
        }
    }
...
const useStore = () => {
    ...
    if(context === undefined) {
        throw new Error('userStore must be used in StoreContext')
    }
